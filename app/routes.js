    ...
    app.get('/query1', function(req, res){
        ...
        async.waterfall([
            ...
            function(callback) {
                ...
                async.forEachOf(periods, function (period, i, foreachcallback) {
...
//                                console.log(JSON.stringify(data));
                ...
                async.forEachOf(initialusers, function (record, i, foreachcallback) {
...
//                            console.log(JSON.stringify(data));
                            ...
                            data.forEach(function(thisuser){
                                ...
                                var usrObj= {queryUserId: user._p_user, queryUserLong: user.longitude, queryUserLat:user.latitude, queryUserBattery: user.battery, queryUserModel: user.model, queryUserVersion: user.version, queryUserAccuracy:user.accuracy, queryUserConfidence: user.confidence, queryUserActivity: user.activity  };
                                var times = {start: record.start, end : record.end};
                                extend(true, thisuser, usrObj, times);
                                console.log(JSON.stringify(usrObj));
                                console.log(JSON.stringify(times));
                                console.log(JSON.stringify(thisuser));
                                current.push(thisuser);
